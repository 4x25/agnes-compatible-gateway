...
- OpenAI file/file-ID video references cannot be converted to an Agnes public
  URL without storage. The MVP supports public HTTP(S) references only.
...
- The only application variable is optional `AGNES_BASE_URL`; it defaults to
  `https://apihub.agnes-ai.com/v1` and must end in `/v1`.
...
- `input_reference.image_url` maps only when it is an HTTP(S) URL. Files,
  `file_id`, and Data URIs are ignored.
- The content endpoint ignores unsupported `variant` values and always resolves
  the video asset.
- Completed content must contain a valid HTTP(S) URL; otherwise the gateway
  returns 502. Redirects use 302, `no-store`, and `no-referrer`.
...
- [x] Normalize one or many image strings without fetching/decoding them.
...
- [x] Map public URL references and ignore unsupported references.
...
upstream bodies, network failures, and an actual OpenAI JavaScript SDK client.
...
The repeatable smoke runner received the temporary key through stdin. The key
was not placed in a command argument, environment variable, file, response log,
or application log.
...
- 2026-07-10: Approved both JSON and OpenAI multipart wire formats for video
  creation.
- 2026-07-10: Approved public URL references for image-to-video; local files and
  `file_id` remain out of scope.
