...
| `POST` | `/v1/images/generations`       | JSON：`model`、`prompt`                  | 省略/`null`/`auto` 尺寸使用 `2048x2048`；可选 `n=1..10` 顺序扇出   |
...
省略 `size`、传入 `null` 或精确传入 `"auto"` 时，网关会向 Agnes 发送
`"2048x2048"`。这是固定的方形回退尺寸，不会根据提示词自动判断宽高比。
...
| 可选     | `size`                                                                                                          | 省略、`null` 或非空白字符串   | 省略、`null` 或精确传入 `"auto"` 时映射为 `"2048x2048"`。其他非空白字符串仍原样转发，且不校验尺寸格式或范围。空字符串、仅含空白的字符串和非字符串会返回本地 `400`。 |
...
- **图像生成：**默认使用 URL 输出。Agnes 当前运行时要求为 `b64_json` 设置两个
  上游标记，网关会自动处理。省略 `size`、传入 `null` 或精确传入 `"auto"` 时使用
  固定的 `"2048x2048"` 回退尺寸。由于真实 Agnes `n=2` 探测未返回两条 URL 结果，
  图片数量大于一时使用上述顺序拆分策略。
- **图像编辑：**仅支持不含 userinfo 的 JSON HTTP(S) URL 和 `image/*` Data URI。
  不支持 multipart 文件、mask 和 `file_id`。网关不会拉取或解码输入。编辑请求仍须
  显式提供 `size`，且该值会原样转发，不使用文生图的回退规则。
