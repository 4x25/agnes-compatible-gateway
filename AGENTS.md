...
- `tests/`: mocked contract tests and an OpenAI JavaScript SDK compatibility
  test.
- `scripts/live_smoke.ts`: opt-in real-Agnes smoke runner that reads a temporary
  key from stdin.
...
- Image edits call Agnes `/images/generations`, normalize `image` to an array,
  and place it under `extra_body.image`. Do not fetch or decode input images in
  the gateway.
- Video durations `4`, `8`, and `12` map to 24 FPS and `97`, `193`, and `289`
  frames. A valid `WIDTHxHEIGHT` maps to integer `width` and `height`.
- Only public HTTP(S) video reference images are mapped. Uploaded files,
  `file_id`, and video Data URIs are ignored.
...
- Do not invent missing video metadata. Content lookup returns 409 while a task
  is pending or failed, 502 for malformed completed output, and a safe 302 for a
  valid completed HTTP(S) URL.
...
- `deno task test:coverage`: generate LCOV/HTML coverage under `coverage/`.
...
- `deno task smoke`: run side-effectful real-Agnes verification using a key
  supplied through stdin.
...
parameter filtering, model pass-through, multipart parsing, Base64 flags,
stateless video polling, safe redirects, malformed upstream responses, and
representative 401/404/429/5xx errors. Update the SDK contract test when a
...
identifier variables internally; these are not application credentials.
...
key. Feed the key to `deno task smoke` through stdin with terminal echo
disabled. Do not automatically retry failed image/video creation; wait for a
capacity or rate-limit window and retry manually only when authorized. After the
run, verify no key remains in the worktree or output and explicitly remind the
user to revoke the temporary key. Generated Agnes assets/tasks may also need
manual cleanup.
